...
def parse_args():
    ...
    parser.add_argument('--size', type=int, default=200, help='Image size (NxN)')
    parser.add_argument('--fov', type=float, default=80.0, help='Field of view in degrees')
    parser.add_argument('--background', type=str, default='images/backgrounds/milky-way-equirec.jpg', help='Background image path')
    parser.add_argument('--steps', type=int, default=200000, help='Number of integration steps for each geodesic (default: 500)')
    parser.add_argument('--delta', type=float, default=0.01, help='Initial integration step size (default: 0.2)')
    ...
    parser.add_argument('--cuda', action='store_true', default=True, help='Enable CUDA GPU acceleration for ray tracing')
    # New simulation configurables
    parser.add_argument('--bh-mass', type=float, default=1.0, help='Black hole mass (default: 1)')
    parser.add_argument('--boundary-radius', type=float, default=31, help='Simulation boundary radius (default: 10*bh_mass)')
    parser.add_argument('--observer-distance', type=float, default=30, help='Observer distance from BH (default: 20*bh_mass)')
    ...
    parser.add_argument('--bg-patch-size-theta', type=float, default=180, help='Background patch size theta (deg, default: 10)')
    parser.add_argument('--bg-patch-size-phi', type=float, default=360, help='Background patch size phi (deg, default: 10)')
    ...
    parser.add_argument('--no-flat-trajectories', action='store_true', default=False, help='Disable calculation and plotting of flat (no-gravity) trajectories in the 3D scene')
